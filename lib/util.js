...
function checkConfig(script, scriptPath, options, callback) {
  ...
  //
  // Turn config.auth into an array:
  //
  if (_.get(script, 'config.auth')) {
    // Is it an object or an array?
    if (_.isArray(script.config.auth)) {
      // an array - nothing to do
    } else if (_.isObject(script.config.auth)) {
      if (options.auth && !_.get(script.config.auth, 'path')) {
        script.config.auth.path = path.resolve(
          process.cwd(),
          options.auth
        );
      } else if (!options.auth && !_.get(script.config.auth, 'path')) {
        console.log(
          'WARNING: config.auth.path not set and auth file not specified with -p'
        );
      } else if (options.auth && _.get(script.config.auth, 'path')) {
        console.log(
          'WARNING - both -p and config.auth.path are set, config.auth.path will be ignored.'
        );
        script.config.auth.path = options.auth;
      } else {
        // no -p but config.payload.path is set - nothing to do
      }

      // Make it an array
      script.config.auth = [script.config.auth];
    } else {
      console.log('Ignoring config.auth, not an object or an array.');
    }
  }

  //
  // Resolve all auth paths to absolute paths now:
  //
  _.forEach(script.config.auth, function(authSpec) {
    const absoluteScriptPath = path.resolve(process.cwd(), scriptPath);
    const resolvedPathToPayload = path.resolve(
      path.dirname(absoluteScriptPath),
      authSpec.path
    );
    authSpec.path = resolvedPathToPayload;
  });
