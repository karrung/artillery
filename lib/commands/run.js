...
function readAuth(script, callback) {
  async.map(
    script.config.auth,
    function readAuthFile(authSpec, next) {
      let data = fs.readFileSync(authSpec.path, 'utf-8');
      const csvOpts = authSpec.options || {};
      csv(data, csvOpts, function(err, parsedData) {
        authSpec.data = parsedData;
        return next(err, authSpec);
      });
    },
    function done(err, results) {
      if (err) {
